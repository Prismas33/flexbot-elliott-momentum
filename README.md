...
# Backtest com estratégia OMDs (EMA 5/21 + MACD 26-55-9)
.\start.ps1 -Action backtest -Symbol 'ETH/USDC:USDC' -Timeframe '15m' -LookbackDays 60 --strategy ema_macd

# Backtest OMDs apenas shorts, aceitando cruzamentos até 16 velas atrás
...
- Use os seletores "Estratégia", "Direção" e "Velas para cruzamento OMDs" para alternar entre **Momentum** ou **OMDs (5/21 & 26-55-9)**, escolher o bias e definir quão recentes os cruzamentos precisam ser (por defeito 8 velas).
...
- O checkbox "Exigir divergência RSI (OMDs)" controla se a confirmação OMDs só aceita setups com divergência; deixe marcado para o comportamento padrão ou desmarque para testar sinais sem esse filtro.
...
- Todas as estratégias respeitam **RR mínimo de 3:1** (recompensa ≥ 3x o risco). A Momentum continua calibrada para ~3R por padrão (stop 1.5× / TP 4.5×). A OMDs agora está configurada para um alvo-base aproximado de 2R (stop 1.8× / TP 3.6×) para experimentação; ainda exigimos que o sinal entregue RR >= 2R para entrada.
